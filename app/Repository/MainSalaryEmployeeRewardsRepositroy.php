...
use App\Models\MainSalaryEmployeeRewards;
use App\Models\Salary_allowances;
use App\Models\salary_sheet;

class MainSalaryEmployeeRewardsRepositroy implements MainSalaryEmployeeRewardsInterface
{
    ...
    public function index()
    {
        ...
        return view('Dashboard.MainSalaryEmployeeRewards.index', compact('get_months'));
    }

    public function show_months($id)
    {
        $com_code = auth()->user()->com_code;
        $get_deductions=Salary_allowances::where('active', 12)->get();
        $finance_per = Finance_cln_periods::where('com_code', $com_code)->where('id', $id)->first();
        $data = MainSalaryEmployeeRewards::all();
        $get_emps = Employment_detail::with('employee')->where('Functiona_status', 1)->get();
        return view('Dashboard.MainSalaryEmployeeRewards.show', compact('data', 'get_emps', 'finance_per','get_deductions'));
    }
    ...
    public function check_exit($request)
    {
        ...
        $get_count = MainSalaryEmployeeRewards::where([
            ['com_code', '=', $com_code],
    ...
    public function store($request)
    {
       if ($request->ajax()){
           ...
           $Absence = MainSalaryEmployeeRewards::create([
    ...
    public function update($request)
    {
      if ($request->ajax()){
          $request->validate([
              'id' => 'required|exists:main_salary_employee_rewards,id',
              'notes' => 'string',
          ...
          try {
              $Absence = MainSalaryEmployeeRewards::findOrFail($request->id);
    ...
    public function print($periodId)
    {
        ...
        $data = MainSalaryEmployeeRewards::with('Employee')
        ...
        return view('Dashboard.MainSalaryEmployeeRewards.print_v1', compact(
    ...
    {
        try {
            $delete_Shift=MainSalaryEmployeeRewards::findorfail($request->id);
            $delete_Shift->delete();
            return redirect()->back()->with('success', 'MainSalaryEmployeeRewards Deleted Successfully');
        }
        catch (\Exception $ex) {
            return redirect()->back()->with('error', 'MainSalaryEmployeeRewards Deleted failed',$ex);
        }
    }
}
