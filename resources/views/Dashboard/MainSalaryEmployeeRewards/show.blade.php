...
@section('content')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title card_title_center">بيانات المكافىات المالية  {{ $finance_per->year_and_month}} </h3>
                <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#createoccasion">
                    <i class="fa fa-plus"></i> إضافة مكافية مالية جديد
                </button>
                <a href="{{ route('MainSalaryEmployeeRewards.print', $finance_per->id) }}"
                   target="_blank"
  ...
  background-color: #f9f9f9;">
        ...
        جاري
                        ...
                        @include('Dashboard.MainSalaryEmployeeRewards.delete')
                        @include('Dashboard.MainSalaryEmployeeRewards.edit')

                    @endforeach
                    </tbody>
                </table>
                @include('Dashboard.MainSalaryEmployeeRewards.create')
...
@section('script')
    <script>
        ...
        $(document).on('click', '#do_action', function(e) {
            ...
            $.ajax({
                url: "{{ route('MainSalaryEmployeeRewards.store') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '#updatePenaltyBtn', function(e) {
            ...
            $.ajax({
                url: "{{ route('MainSalaryEmployeeRewards.update') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
