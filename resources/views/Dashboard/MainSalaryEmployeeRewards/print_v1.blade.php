...
<head>
    <meta charset="utf-8">
    <title>تقرير المكافآت المالية </title>
...
<table style="width: 60%; float: right; margin-right: 5px;" dir="rtl">
    ...
    <tr>
        <td style="text-align: center; padding: 10px; font-weight: bold;">
            <span style="display:inline-block;width:600px;height:40px;text-align:center;color:blue;border:1px solid #000;">
                تقرير بسجل المكافآت المالية الراتب الموظفين عن الشهر المالي:
                {{ $month ?? '---' }} سنة {{ $year ?? '---' }}
...
@if(isset($data) && count($data)>0)
    <table dir="rtl" class="table table-bordered table-hover" style="width: 99%; margin: 0 auto;">
        ...
        <th style="width: 15%;">نوع الراتب الاضافي</th>
        ...
        @endforeach
        <tr>
            <td colspan="4" style="background-color:lightsalmon; text-align:center; font-weight:bold;">
                الإجمالي
            </td>

            <td colspan="3" style="background-color: lightgreen; text-align: right;">
