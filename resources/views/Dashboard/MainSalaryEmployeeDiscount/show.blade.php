    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title card_title_center">بيانات الخصومات المالية  {{ $finance_per->year_and_month}} </h3>
                <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#createoccasion">
                    <i class="fa fa-plus"></i> إضافة خصم مالي جديد
                </button>
                <a href="{{ route('MainSalaryEmployeeDiscount.print', $finance_per->id) }}"
                   target="_blank"
        ...
        جاري
                            ...
                            <td>
                                <button
                                    ...
                                    data-deduction_types_id="{{ $info->deduction_types_id }}"
                        ...
                        @include('Dashboard.MainSalaryEmployeeDiscount.delete')
                        @include('Dashboard.MainSalaryEmployeeDiscount.edit')

                    @endforeach
                    </tbody>
                </table>
                @include('Dashboard.MainSalaryEmployeeDiscount.create')
...
@section('script')
    <script>
        ...
        $(document).on('click', '#do_action', function(e) {
            ...
            let deduction_types_id = $('#deduction_types_id').val();
            ...
            $.ajax({
                url: "{{ route('MainSalaryEmployeeDiscount.store') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '#updatePenaltyBtn', function(e) {
            ...
            let deduction_types_id = $('#edit_deduction_types_id').val();
            let total = $('#edit_total').val();
            let notes = $('#edit_notes').val();


            $.ajax({
                url: "{{ route('MainSalaryEmployeeDiscount.update') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
