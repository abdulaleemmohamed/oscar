...
@section('content')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title card_title_center">بيانات البدلات المالية  {{ $finance_per->year_and_month}} </h3>
                <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#createoccasion">
                    <i class="fa fa-plus"></i> إضافة بدل مالي جديد
                </button>
                <a href="{{ route('MainSalaryEmployeeAllowances.print', $finance_per->id) }}"
                   target="_blank"
  ...
  background-color: #f9f9f9;">
        ...
        جاري
                            ...
                            <td>
                                <button class="btn btn-info btn-sm editAbsenceBtn"
                                        data-toggle="modal"
                                        data-target="#editAbsenceModal"
                                        data-id="{{ $info->id }}"
                                        data-total="{{ $info->total }}"
                                        data-allowance_types_id="{{ $info->allowance_types_id }}"
                        ...
                        @include('Dashboard.MainSalaryEmployeeAllowances.delete')
                        @include('Dashboard.MainSalaryEmployeeAllowances.edit')

                    @endforeach
                    </tbody>
                </table>
                @include('Dashboard.MainSalaryEmployeeAllowances.create')
...
@section('script')
    <script>
        ...
        $(document).on('click', '#do_action', function(e) {
            ...
            let allowance_types_id = $('#allowance_types_id').val();
            ...
            $.ajax({
                url: "{{ route('MainSalaryEmployeeAllowances.store') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '.editAbsenceBtn', function (e) {
            ...
            // تعبئة الحقول
            $('#penalty_id').val(penaltyId);let id = $(this).data('id');
        ...
        $(document).on('click', '#updatePenaltyBtn', function(e) {
            ...
            let allowance_types_id = $('#edit_allowance_types_id').val();
            let total = $('#edit_total').val();
            let notes = $('#edit_notes').val();


            $.ajax({
                url: "{{ route('MainSalaryEmployeeAllowances.update') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
